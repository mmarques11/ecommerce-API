...
def login():
    ...
    return jsonify({"message": "Não autorizado, crendenciais invalidas"}), 401
...
def add_product():
    ...
    return jsonify({"message": "Não foi possível cadastrar produto."}), 400
...
#Rota para adicinar item ao carrinho
...
def checkout():
    ...
    for cart_item in cart_items:
        db.session.delete(cart_item)
    db.session.commit()
    return jsonify({'message': 'Checkout successfull. Carrinho foi limpo'})   
